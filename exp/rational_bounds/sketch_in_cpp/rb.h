...
namespace rational_bounds {
	...
	// rational bounds interval characteristics
	namespace interval {
		r64 length(const rb32& x) {
			return sub(x.ub, x.lb);
		}

		bool degenerate(const rb32& x) {
			const auto len = sub(x.ub, x.lb);
			return len.n == 0 && len.d != 0;
		}

		bool left_bound(const rb32& x) {
			return x.lb.d != 0;
		}

		bool right_bound(const rb32& x) {
			return x.ub.d != 0;
		}

		bool finite(const rb32& x) {
			return left_bound(x) && right_bound(x);
		}
	}
}
